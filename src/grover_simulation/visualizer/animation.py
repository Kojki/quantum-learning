...
def run(
    ...
    Args:
        ...
        threshold: コストのしきい値。target_bitstrings が指定された場合は oracle 構築のみに使用。
        ...
        target_bitstrings: 正解のビット列リスト。指定時は threshold から計算せずこれを使用。
            これにより浮動小数点誤差による「正解なし→成功率0%」を防げる。
    """
    ...
    # 渡されない場合は threshold から計算する
    if target_bitstrings is None:
        target_bitstrings = _enumerate_targets(n_qubits, condition)
    else:
        # oracle は threshold で作成済みだが、成功率の計算は渡された target を使う
        print(
            f"  [animation] target_bitstrings を直接受け取りました: {len(target_bitstrings)} 件"
        )
