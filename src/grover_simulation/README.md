...
- **地名から座標への自動変換**：地名を入力すると OpenStreetMap から座標を取得し、実距離を自動計算
- **可視化**：確率変化アニメーション・古典 量子比較・地図上のルート描画
...
│   └── simulator.py         # シミュレータ（未実装）
│
├── classical/               # 古典アルゴリズム
│   ├── brute_force.py       # 全探索
│   └── heuristic.py         # ヒューリスティック探索（未実装）
│
├── benchmark/               # ベンチマーク・評価
│   └── metrics.py           # 量子・古典アルゴリズムの性能指標算出
│
├── visualizer/              # 可視化
│   ├── core.py              # 共通ユーティリティ（フォント・配色・ファイル保存）
│   ├── animation.py         # 確率変化アニメーション（理想 vs ノイズあり）
│   ├── landscape.py         # 成功確率の 3D サーフェス描画（未実装）
│   └── state_plotter.py     # 量子状態の可視化（未実装）
│
├── geo/                     # 地図・座標関連
│   ├── geocoder.py          # 地名から緯度経度への変換（geopy 使用）
│   ├── distance.py          # 2 点間の実距離計算（Haversine 公式）
│   └── map_plotter.py       # 地図上へのルート描画（contextily 使用）
│
├── data/                    # 実験データの管理（未実装）
│   ├── experiments.py
│   └── persistence.py
│
├── animation/               # アニメーションの実行（未実装）
│   └── runner.py
│
├── docs/                    # ドキュメント
